...
import {frame, gn, CSSTransition, localx, newHTML, scaleMultiplier, fullscreenScaleMultiplier,
    ...
    setProps, globalx, OS} from '../../utils/lib';
...
export default class UI {
    ...
    static addProjectInfo () {
        ...
        if (window.Settings.shareEnabled) {
            ...
            window[OS].deviceName(function (name) {
    ...
    static infoDoShare (evt, nameField, shareLoadingGif, shareType) {
        ...
        window[OS].analyticsEvent('editor', 'share_button', (shareType == EMAILSHARE) ? 'email' : 'airdrop');
        ...
        function saveAndShare () {
            ...
            Project.prepareToSave(ScratchJr.currentProject, function () {
                ...
                IO.zipProject(ScratchJr.currentProject, function (contents) {
                    ...
                    window[OS].sendSjrToShareDialog(IO.zipFileName, emailSubject, Localization.localize('SHARING_EMAIL_TEXT'),
                        shareType, contents);
    ...
    static finishTextFieldSave (dontHide) {
        ...
        window[OS].setfield(window[OS].database, Project.metadata.id, 'name', pname);
        if (!dontHide) {
            ScratchAudio.sndFX('exittap.wav');
    ...
    static switchGrid () {
        ...
        window[OS].analyticsEvent('editor', Grid.hidden ? 'hide_grid' : 'show_grid');
    }
